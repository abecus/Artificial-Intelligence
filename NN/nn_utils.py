...
def model_forward(X, parameters):
    ...
    for l in range(1, L):
        A_prev = A 
        A, cache = linear_activation_forward(A_prev, parameters['W' + str(l)], parameters['b' + str(l)], activation = "leaky_relu")
        caches.append(cache)
...
def model_backward(AL, Y, caches):
    ...
    Arguments:
    AL -- probability vector, output of the forward propagation (model_forward())
    ...
    for l in reversed(range(L-1)):
        ...
        dA_prev_temp, dW_temp, db_temp = linear_activation_backward(grads["dA" + str(l + 2)], current_cache, activation = "leaky_relu")
...
def update_parameters(parameters, grads, learning_rate):
    ...
    # Update leaky_rule for each parameter. Use a for loop.
    for l in range(L):
...
def multi_class_predict(X, y, parameters):
    """
...
def binary_class_predict(X, y, parameters):
    """
    This function is used to predict the results of a  L-layer neural network.
